...
**Architecture in one sentence:** ADRs in `docs/adr/` are the single write path for durable decisions; instruction files, golden files, and lint config are generated or derived projections; probe prompts and CI gates verify compliance; a task-closeout distill step feeds new decisions back into the ADR store.
...
3. **Migrate existing decisions.** Every durable decision from the Phase 0 inventory gets an ADR. One decision per file. Anything that can't be expressed as a single atomic `rule` is probably two ADRs.
...
**Exit criteria:** All durable decisions live as schema-valid ADRs; `adr-lint` passes in CI; no decision reasoning remains hand-authored anywhere else.
...
1. **Generator script** (`scripts/project-adrs`, plain Node or Python, repo-native):
   ...
   - Emits `docs/adr/index.md`: one line per ADR (`id | title | status | rule`). This is the cheap always-loadable map.
   - Emits/updates a fenced, generated block in `.github/copilot-instructions.md` containing: the ADR index reference, the ≤5–7 truly universal rules (those with `scope: **`), and a standing instruction: *"Before modifying files matching an ADR's scope, read the full ADR in docs/adr/. Do not re-litigate decisions with status: accepted; rejected alternatives are listed in each record."*
...
## Phase 3 — Push decisions down the enforcement stack (2–4 days, ongoing)
...
3. **Track the demotion ratio:** count of rules enforced mechanically vs. by prose. The ratio should rise over time; a stagnant ratio means new decisions are defaulting to the weakest layer.
...
- **ADR overhead deters capture.** Mitigation: the distill step drafts candidates from scratch content the agent already wrote; human effort is accept/edit/reject, not authoring from scratch. Reserve ADRs for decisions that are expensive to reverse, likely to be second-guessed, or counterintuitive.
...
5. Measured reduction in decision-churn rework (agent-undone decisions caught in review), acknowledging defect rates may be unchanged.
