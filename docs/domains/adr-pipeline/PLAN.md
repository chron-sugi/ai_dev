...
5. **Push-path protection (already designed, verify in place):** `PreToolUse` hook denying `git push` while `.velocai/` is tracked, `SessionEnd` reminder, plain git `pre-push` hook for manual pushes.
